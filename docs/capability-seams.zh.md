...
flowchart LR
  ...
  pkg_git["git"]
  svc_git["ctx.git<br/>Git query seam"]
...
| `ctx.git` | `seam` | [`git`](../packages/git/git) | [`git-local`](../packages/git/git-local) | [`tool-git`](../packages/git/tool-git) | - | 只读仓库查询（status、diff、log）通过本地 CLI 后端运行；tool-git 拥有面向模型的工具。 |

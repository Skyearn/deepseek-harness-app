...
| `ctx.git` | `seam` | [`git`](../packages/git/git) | [`git-local`](../packages/git/git-local) | [`tool-git`](../packages/git/tool-git) | - | Read-only repository queries (status, diff, log) run through the local CLI backend; tool-git owns the model-facing tools. |
